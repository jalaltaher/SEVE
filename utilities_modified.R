...
transactions <- function(df,train_starts,train_ends,tz){
    ...
    select(Start_Date,Arrival,Charge.Duration,Energy,Park.Duration,dow)
    return(res)
}
...
gmm_fit <- function(df, max_sample = NULL, verbose = TRUE) {
    ...
    # Split data by 'dow'
    df_split <- split(df %>% select(Arrival,Charge.Duration,Energy,Park.Duration), df$dow)
    ...
    names(gmm_models) <- paste0("day_", names(df_split))
...
sarima_simulate <- function(fit,sess,horizon,n_sim,freq=7,reg=TRUE, verbose=TRUE){
...
for (i in 1:n_sim) {
  ...
  for (t in 1:horizon) {
    ...
    sim <- simulate(fit, nsim = 1, future = TRUE, bootstrap = TRUE)
    simulated_series[t] <- sim
    fit <- Arima(c(X, simulated_series[1:t]), model = fit ) # Refit model with simulated series
        }
  }
...
relevant_sim <- function(gmm_model,n_sample){
    ...
    while (j<n_sample){
        ...
        colnames(temp) <- c('Arrival','Charge.Duration','Energy','Park.Duration')
        trans_sim <- bind_rows(temp,trans_sim) 
        trans_sim <- trans_sim %>% 
        filter(Arrival >0,Arrival<24,Charge.Duration>0,Charge.Duration<60*24,Energy>0,Energy<100,Park.Duration > 0, Park.Duration < 60 * 48,
  Park.Duration > Charge.Duration)
...
gmm_simulate <- function(gmm,quantile_forecasts,n_simu,verbose=TRUE){
    ...
    print("===== GMM simulation modified =====")
    start <- Sys.time()

    horizon <- nrow(quantile_forecasts)
    ...
    gmm_indices <- quantile_forecasts$weekday
    start_dates <- quantile_forecasts$Start_Date
    
    for (quantile_index in seq_along(quantiles)) {
      q <- quantiles[quantile_index]
      
      # Extract once
      n_samples   <- quantile_forecasts[[q]]
      simu_list <- vector("list", n_simu)
      
      for (n in seq_len(n_simu)) {
        temp_list <- vector("list", horizon)
        
        for (t in seq_len(horizon)) {
          result <- relevant_sim(gmm[[ paste0('day_',gmm_indices[t]) ]], n_samples[t])
          
          if (!is.null(result)) {
            # Attach date column
            result$Start_Date <- start_dates[t]
          } else {
            # Zero‐row placeholder with correct columns:
            result <- data.frame(
              Arrival         = NA,
              Charge.Duration = 0,
              Energy          = 0,
        ...
        # Even if some temp_list elements are zero‐row, bind_rows keeps them as empty blocks
...
plot_energy <- function(final){
  p <- ggplot(final, aes(x = Start_Date)) +
  geom_ribbon(aes(ymin = q10, ymax = q90, fill = "q10-q90"), alpha = 0.4) +
  geom_line(aes(y = q50, color = "Median (q50)"), linewidth = 1.5, linetype = "solid") +
  geom_line(aes(y = mean, color = "Mean"), linewidth = 1.2, linetype = "dashed") +
  scale_color_manual(
    name = "Lines",
    values = c("Mean" = "black", "Median (q50)" = "#0072B2")
  ) +
  scale_fill_manual(
    name = "Ribbon",
    values = c("q10-q90" = "purple")
  ) +
  labs(
    title = "Distribution Over Time: Median, Mean, and Spread (q10-90)",
    x = "Date",
    y = "Energy (kWh)"
  ) +
